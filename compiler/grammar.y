%{
	...
	void codeGenOp(node *opNode)
	{
		...
		if((!strcmp(opNode->NType, "If")) || (!strcmp(opNode->NType, "Elsif")))
		{			
			switch(opNode->noOps)
			{
				case 2 : 
		...
		if(!strcmp(opNode->NType, "require"))
		{
			printf("require %s\n", opNode->NextLevel[0]->id->name);
			makeQ("-", opNode->NextLevel[0]->id->name, "-", "require");
			return;
		}
...
%}
    // Add Ruby-specific tokens here

    %union { char *text; int depth; struct ASTNode* node; };

    %locations

    %token T_EndOfFile T_Return T_End T_Number T_True T_False T_ID T_Puts T_Cln T_EL T_NL T_EQL T_NEQ T_EQ T_GT T_LT T_EGT T_ELT T_Or T_And T_Not T_In ID ND DD T_String T_If T_Elsif T_While T_Else T_Require T_Break T_Pass T_MN T_PL T_DV T_Do T_Rescue T_Retry T_Ensure T_Begin T_Case T_When T_Then T_ML T_OP T_CP T_OB T_CB T_Def T_Comma T_List
    ...
    %type<node> StartDebugger args start_suite suite end_suite func_call call_args StartParse finalStatements arith_exp bool_exp term constant basic_stmt cmpd_stmt func_def list_index require_stmt pass_stmt break_stmt puts_stmt if_stmt elsif_stmts else_stmt while_stmt return_stmt assign_stmt bool_term bool_factor
    ...
    require_stmt : T_Require T_ID {insertRecord("PackageName", $<text>2, @2.first_line, currentScope); $$ = createOp("require", 1, createID_Const("PackageName", $<text>2, currentScope));};
    ...
    puts_stmt : T_Puts term {$$ = createOp("Puts", 1, $2);};
    ...
    if_stmt : T_If bool_exp start_suite {$$ = createOp("If", 2, $2, $3);}
            | T_If bool_exp start_suite elsif_stmts {$$ = createOp("If", 3, $2, $3, $4);};

    elsif_stmts : else_stmt {$$= $1;}
               | T_Elsif bool_exp start_suite elsif_stmts {$$= createOp("Elsif", 3, $2, $3, $4);};
    ...
    end_suite : T_End {updateCScope($<depth>1);} finalStatements {$$ = createOp("EndBlock", 1, $3);} 
              | T_End {updateCScope($<depth>1);} {$$ = createOp("EndBlock", 0);}
